 ...
 *
 * @param ucs - Unicode code point to evaluate.
 * @returns The display width of `ucs` in terminal columns.
 */
export function wcwidth(ucs: number): number {
  ...
  function nonCombining(ucs: number): number {
    ...
    return bisearch(ucs, table);
  }

  return 1 + nonCombining(ucs);
}
...
export function wcswidth(str: string, n?: number): number {
  ...
  for (let c = 0; c < n; c++) {
    const ucs = pwcs[c].codePointAt(0);

    if (ucs === undefined || (w = wcwidth(ucs)) < 0)
      return -1;
    else
      width += w;
  }
 ...
 *
 * @param ucs - Unicode code point.
 * @returns Column width in terminal cells, or -1 on error.
 */
export function wcwidthCjk(ucs: number): number {
...
export function wcswidthCjk(str: string, n?: number): number {
  ...
  for (let c = 0; c < n; c++) {
    ...
    if (ucs === undefined || (w = wcwidthCjk(ucs)) < 0)
      return -1;
    else
      width += w;
  }
